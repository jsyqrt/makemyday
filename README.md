...
**Make My Day** 是一个纯前端的 AI 日程规划助手。用户可以通过**文字或语音**输入计划，AI 会智能地将这些计划分解成具体可操作的事件，并根据**"紧急重要四象限"**理论给出优先级建议和行动方案。

### 核心特点

- 🤖 **AI 智能规划**：自动解析计划，生成具体事件和建议
- 🎯 **四象限管理**：科学的时间管理方法
- 🎤 **语音输入**：支持 SiliconFlow API 和浏览器内置识别
...
- ✅ 识别准确度高（FunAudioLLM/SenseVoiceSmall 模型）
- ✅ 录音完成后自动转文字
- 📝 使用：点击"语音"按钮开始录音，再次点击停止并识别
...
- **SiliconFlow API**：语音识别（可选）
- **Web Speech API**：浏览器语音识别（备选）
...
### 语音识别
- ✅ **推荐**：配置 SiliconFlow 语音 API，支持所有浏览器
- ⚠️ 浏览器内置识别仅支持 Chrome、Edge
- ⚠️ 需要麦克风权限
- ⚠️ 生产环境需要 HTTPS
...
**解决方案**：
1. **推荐**：配置 SiliconFlow 语音 API Key（与 LLM API Key 相同）
2. 确保授予浏览器麦克风权限
3. 如使用浏览器内置，确保是 Chrome 或 Edge
4. 生产环境确保使用 HTTPS
